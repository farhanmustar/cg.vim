...
let g:cg_comp_temperature = get(g:, 'cg_comp_temperature', 0.3)  " 0 - 2 -> higer value less predictable
...
function! s:send_query(cmd, buf_nr) abort
  echo 'CG querying for response...'
  if has('job')
    let Callback = function('s:job_callback', [a:buf_nr])
    call job_start(a:cmd, {'close_cb': Callback})
  elseif has('nvim')
    let Callback = function('s:job_callback_nvim', [a:buf_nr])
...
function! s:job_callback(buf_nr, channel) abort
  let l:response = []
  while ch_status(a:channel, {'part': 'out'}) == 'buffered'
    let l:response += [ch_read(a:channel)]
  endwhile
  call s:process_comp_response(l:response, a:buf_nr)
endfunction

function! s:job_callback_nvim(buf_nr, id, data, event) abort
  call s:process_comp_response(a:data, a:buf_nr)
endfunction

function! s:process_comp_response(response, buf_nr) abort
...
function! s:fill(content) abort
  ...
  if empty(a:content)
    execute ':normal! IFail to get response or response empty.'
    silent normal! gg
  else
